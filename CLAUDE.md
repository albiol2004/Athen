...
- [Self-Support Skills](docs/SELF_SUPPORT_SKILLS.md) — Read when: working on Athen-as-IT-support: per-provider help modals (L1, ships first, no LLM needed), `skills/system/` knowledge-only walkthroughs (L2, reuses shipped SkillStore + adds `system: true` flag), Skill-gated structured-settings Tauri commands (L3, deferred), `cargo xtask gen-skills` local generation pipeline + CI staleness check, and proactive post-onboarding help sense. Design doc; not yet implemented. Builds on shipped SKILLS.md.
