...
- [Wake-ups](docs/WAKEUPS.md) — Read when: building scheduled / recurring / one-shot proactive triggers (reminders, daily digests, deferred follow-ups). Wake-ups are synthetic sense events with a clock as their trigger; they reuse coordinator/risk/dispatch. Risk model is "pre-approve capability, not content" via per-wake-up `AutonomyBand` + tool/contact allowlists, with the existing per-action risk gate still firing at fire time. Design doc; not yet implemented.
