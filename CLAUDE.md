...
- [Code Mode](docs/CODE_MODE.md) — Read when: working on the per-arc, UI-only "Code Mode" posture for power-user software work (real git repo rooted on the arc, in-place by default, opt-in `git worktree` per parallel sub-agent, read-only git recognition + worktree/WIP visualization, scoped live agent activity). DESIGN + BUILD IN FLIGHT (2026-06-25, `feat/code-mode`). Orthogonal to agent profiles (profile = who the agent is; Code Mode = what environment the arc runs in); the two compose. NEVER set by `sense_router` — desktop/web client only, so sense-created arcs are always `code_mode=false`. Per-arc `code_mode`+`code_mode_root` columns on `arcs` (ArcMeta, mirrors `project_id`/`research_*` adds). Introduces the FIRST per-arc cwd: `working_dir_override`+`fs_base_override` on `ToolRegistryState` so shell cwd + `write`/`edit`/`read`/`save_file` relative resolution + sandbox `allowed_paths` all anchor at the repo root (move together or `ls`≠`write`). D5: when Code Mode + a real repo is detected, the shadow `GixCheckpointStore` snapshot is SKIPPED for that arc and real git becomes the undo/Changes surface (Revert-button (a) lean-on-git vs (b) keep-shadow is the one OPEN question — leaning (a)). Recognition layer `athen-app/src/code_mode.rs` shells the system `git` binary read-only (porcelain parse: `worktree list`/`status`/`log`/`diff`; `gix 0.83` has no worktree/status features) → `GitRepoState` → `code_mode_git_state` cmd + `GET /api/arcs/{id}/code-mode/git`. Agent DRIVES git (incl. worktree creation) via shell; Athen only recognizes+visualizes. Agent viz = existing agent-control page scoped to arc/project, live; sub-agent transcripts stay ephemeral (only final output persisted — existing delegation contract). GithubIdentity reuses the per-profile `GH_TOKEN`/author/committer/`GH_CONFIG_DIR` injection unchanged. Code is authoritative once shipped.
