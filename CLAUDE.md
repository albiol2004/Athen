...
- `athen-caldav` — CalDAV adapter implementing `athen_core::traits::calendar_source::CalendarSource`. One generic struct (`CalDavSource`) speaks RFC 4791 over HTTPS with HTTP Basic auth + an app-specific password. Same code handles iCloud, Google Calendar via its CalDAV endpoint (sidesteps Google OAuth verification entirely), Fastmail, Nextcloud, Yandex. Producer side of the pipeline: a sync loop in `athen-app` (task #26) pulls `RemoteEvent`s from each configured source and reconciles them with the existing `CalendarStore` (`athen-persistence/src/calendar.rs`) on the `(source_id, remote_id)` key. Wire protocol (PROPFIND/REPORT/PUT/DELETE bodies + iCalendar parse) is **stubbed pending task #25** — methods return `AthenError::Other("not yet wired")`. `presets` module holds per-provider base URLs + credential-page hints used by the Settings UI (task #27).
