...
- [Projects](docs/PROJECTS.md) — Read when: working on Projects (the ChatGPT/Claude-style container that groups many arcs around common work), the opinionated workspace folder layout (`UserInfo/`/`Downloads/`/`Projects/<name>/`/...), the `save_file` category tool, or project-wide compaction. SHIPPED 2026-06-20 (§7 steps 1–3; only sense auto-routing deferred). A Project is a context-scope above arcs — an id that arcs (`arcs.project_id`), a workspace folder (`Projects/<folder_slug>/`, renamed when the project is renamed), and a maintained summary all hang off. `ProjectStore` (athen-persistence/src/projects.rs: projects + project_arc_folds + project_meta tables). Project **instructions live in a dedicated `instructions` column** (NOT the doc's original Identity `ProfileTag::Project` sketch). Context sharing layered cheapest-first: instructions (cached prefix via executor `project_block`) → file listing + scoped memory recall + summary (volatile system_suffix). Project summary = incremental hierarchical compaction (`LlmProjectCompactor.fold_arc_into_project`, min-delta gate, cheap Fast tier, fired best-effort at arc transitions) with a **persisted** `summary_mode` (auto/manual/off) for token-averse/local users. CRUD via Tauri commands + `/api/projects*` HTTP routes + desktop panel + web `PanelProjects.tsx`. `save_file(category, project, filename, content)` resolves the bucket path in code then delegates to `write`. Sense auto-routing into projects is future work building on Multi-Intent Routing (#152). Code is authoritative.
- [Deep Research](docs/DEEP_RESEARCH.md) — Read when: working on the triggered long-running research workflow (decompose a question into N sub-questions → fan out N concurrent worker sub-agents that each `web_search`+`web_fetch` ~5 sources → synthesize a cited markdown paper saved into the workspace, with in-app "View paper", follow-up Q&A, and extend-vs-new on re-trigger). SHIPPED 2026-06-25 (`feat/deep-research`, button/command trigger). Orchestrator `crates/athen-app/src/deep_research.rs` (`run_deep_research`: plan/fan-out/synthesize, decoupled from AppState via a worker-spawn closure + progress callback) built on the executor's `join_all` fan-out seam by reusing `run_delegation` (NOT a new concurrency engine; the coordinator's parallel dispatch is external-event-only). Orchestrator adds the missing `tokio::sync::Semaphore` cap + partial-failure tolerance. Depth budget quick/standard/deep (3/5/8 workers). `deep_research_core` + `deep_research` Tauri cmd + `POST /api/arcs/{id}/deep-research`; paper saved via `save_file` (Outputs/), arc metadata `research_paper_path`/`research_question` drives extend-vs-new; `get_research_paper` + `GET /api/arcs/{id}/research-paper` read-by-arc-id (no path-traversal surface). `deep_research_worker` built-in profile (web/files/memory groups). Desktop + web UIs at parity (trigger dialog, progress banner via `deep-research-progress`/`-done` events, result card, View-paper markdown modal). Both triggers shipped: the UI/command/HTTP path AND the agent-callable `deep_research` tool (an optional `DeepResearchRunner` closure on `AppToolRegistry` that resolves `&AppState` via `ui.app_state()` at call-time, injected ONLY into the top-level `assemble_app_tool_registry` — sub-agent/worker/save-only registries get `None`, so workers can't recurse and the addition is purely additive; extend-vs-new handled in-band via a `needs_decision` tool result). ONE DIVERGENCE from the doc: decentralized — each worker owns a sub-question and finds its own ~5 sources, NOT central discover-then-partition. Code is authoritative.
- [Code Mode](docs/CODE_MODE.md) — Read when: working on the per-arc, UI-only "Code Mode" posture for power-user software work (real git repo rooted on the arc, in-place by default, opt-in `git worktree` per parallel sub-agent, read-only git recognition + worktree/WIP visualization, scoped live agent activity). DESIGN + BUILD IN FLIGHT (2026-06-25, `feat/code-mode`). Orthogonal to agent profiles (profile = who the agent is; Code Mode = what environment the arc runs in); the two compose. NEVER set by `sense_router` — desktop/web client only, so sense-created arcs are always `code_mode=false`. Per-arc `code_mode`+`code_mode_root` columns on `arcs` (ArcMeta, mirrors `project_id`/`research_*` adds). Introduces the FIRST per-arc cwd: `working_dir_override`+`fs_base_override` on `ToolRegistryState` so shell cwd + `write`/`edit`/`read`/`save_file` relative resolution + sandbox `allowed_paths` all anchor at the repo root (move together or `ls`≠`write`). Undo (§6, RESOLVED option b): the shadow `GixCheckpointStore` STAYS active in Code Mode so agent write/edit/shell keep per-action file-level undo (Changes rail); real git is the *visualization + manual-discard* surface, not a replacement. GitLens-style discard: `code_mode::discard_path` (tracked→`git restore --staged --worktree --source=HEAD`; untracked→`git clean -fd`) / `discard_all` (`git checkout -- .`+`clean -fd`), path lexically fenced to root, UI-only + confirm-gated + NOT agent-callable, via `code_mode_discard` cmd + `POST /api/arcs/{id}/code-mode/discard` returning fresh `GitRepoState`. Recognition layer `athen-app/src/code_mode.rs` shells the system `git` binary read-only (porcelain parse: `worktree list`/`status`/`log`/`diff`; `gix 0.83` has no worktree/status features) → `GitRepoState` → `code_mode_git_state` cmd + `GET /api/arcs/{id}/code-mode/git`. Agent DRIVES git (incl. worktree creation) via shell; Athen only recognizes+visualizes. Agent viz = existing agent-control page scoped to arc/project, live; sub-agent transcripts stay ephemeral (only final output persisted — existing delegation contract). GithubIdentity reuses the per-profile `GH_TOKEN`/author/committer/`GH_CONFIG_DIR` injection unchanged. Code is authoritative once shipped.
