...
- [Deep Research](docs/DEEP_RESEARCH.md) — Read when: working on the triggered long-running research workflow (decompose a question into sub-questions → fan out N concurrent worker sub-agents that each `web_search`+`web_fetch` ~5 sources → synthesize a cited markdown paper saved into the workspace, with follow-up Q&A and extend-vs-new on re-trigger). DESIGN ONLY 2026-06-25 (audit-grounded). Key audited facts: parallel fan-out already works via the executor's `join_all` over batched tool calls (`executor.rs:2473`) — build an orchestrator on that seam (reuse `run_delegation`), NOT a new concurrency engine; the coordinator's parallel dispatch is external-event-only and unreachable from a tool call. Workers use `web_search` (`tools.rs`) + `web_fetch` (the page reader, `HybridReader` backend). Gaps the orchestrator must fill: NO concurrency cap/semaphore exists, and the in-batch dedup guard stubs identical calls so fan-out briefs must be distinct. First-class orchestrator (`crates/athen-app/src/deep_research.rs`) over LLM-improvised batching for determinism/partial-tolerance/cost-cap/lifecycle. Depth budget (quick/standard/deep), modify-vs-new as arc metadata (`arcs.research_paper_path`+`research_question`), paper saved via `save_file`. Code is authoritative once it ships.
