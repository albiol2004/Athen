...
- Schema: `tasks`, `task_steps`, `checkpoints`, `pending_messages`, `arcs`, `arc_entries`, `calendar_events`, `fired_reminders`, `contacts`, `contact_identifiers`, `notifications`, `grants`, `enabled_mcps`, `chat_messages` (legacy), `chat_sessions` (legacy) tables.
...
- `tools.rs`: `ShellToolRegistry` implementing `ToolRegistry` with 6 built-in tools (extended to 10 by `AppToolRegistry` in athen-app). Provides `ShellExtraWritableProvider` trait for per-arc directory grant discovery (wired by `file_gate.rs`):
  ...
  Each tool has proper JSON Schema parameter definitions for LLM tool calling. `ShellToolRegistry` holds an optional `UnifiedSandbox` (from `athen-sandbox`), a `RuleEngine` (from `athen-risk`), and optional `ShellExtraWritableProvider` -- all auto-initialized on construction.
- `tool_grouping.rs`: Helpers for 2-tier tool discovery. `group_for(name)` extracts the prefix (e.g. "memory", "calendar", "files" from tool names). `summarize_groups()` returns one-line summary per group for the system prompt.
