...
- `src/commands.rs::persist_uploaded_attachments`: Composer-side companion to the email/Telegram persist paths. Decodes base64 from the frontend `attachments` IPC param, writes each file to `~/.athen/sense-attachments/<event_id>/<sanitized_name>`, inserts an `AttachmentStore` row with `source = None` (local upload, no refetch), eagerly extracts PDF text via `spawn_blocking`, and stamps a `system` arc entry whose metadata carries the synthesized `event_id` so `latest_sense_event_id_in_arc` picks it up. From there the existing `prepare_attachment_surfacing` block in `execute_approved_task` does the rest. 5 unit tests cover empty-uploads short-circuit, no-store fallback, text upload + arc metadata stamp, and filename sanitization (path traversal escapes).
