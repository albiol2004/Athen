...
- `toolbox.rs`: persistent shell-tool memory. `Runtime::{Python, Node}`, `InstalledPackage`, `ToolboxManifest` (atomic save), `RuntimeProbe` (`python`/`pip`/`node`/`npm` detection with 5s timeouts, cached in process-scoped `OnceCell`), `ToolboxPromptInfo` (probe + manifest summary injected into the executor system prompt). `install_python_package`/`install_node_package` shell out to `pip install --target=<dir>` / `npm install --prefix=<dir>`. `uninstall_python_package` walks `dist-info/RECORD` (PEP 503-normalized) since `pip uninstall` does not support `--target`; `uninstall_node_package` uses `npm uninstall --prefix`. `clear_toolbox` wipes the directory and manifest.
